...
bool seed_scan_normalPID_unroll16_AVX512(size_t rolls, uint32_t desired_pid, uint64_t start_seed, uint64_t iterations);
...
bool seed_scan_maxPID_unroll16_AVX512(size_t rolls, uint32_t desired_pid, uint64_t start_seed, uint64_t iterations);
...
bool seed_scan_normalPID(size_t rolls, uint32_t desired_pid, uint64_t start_seed, uint64_t iterations){
    ...
    if (CPU_CAPABILITY.OS_AVX512 && CPU_CAPABILITY.HW_AVX512_DQ){
        ...
        if (block > 0 && seed_scan_normalPID_unroll16_AVX512(rolls, desired_pid, start_seed, block)){
            return true;
        }
    ...
    if (CPU_CAPABILITY.OS_AVX && CPU_CAPABILITY.HW_AVX2){
        ...
        if (block > 0 && seed_scan_normalPID_unroll8_AVX2(rolls, desired_pid, start_seed, block)){
            return true;
        }
        start_seed += block * 0x100000000;
        iterations -= block;
    ...
    if (CPU_CAPABILITY.HW_SSE41){
        ...
        if (block > 0 && seed_scan_normalPID_unroll4_SSE41(rolls, desired_pid, start_seed, block)){
            return true;
        }
        start_seed += block * 0x100000000;
        iterations -= block;
    ...
    if (iterations > 0){
        return seed_scan_normalPID_Default(rolls, desired_pid, start_seed, iterations);
    }
    return false;
}
bool seed_scan_maxPID(size_t rolls, uint32_t desired_pid, uint64_t start_seed, uint64_t iterations){
    ...
    if (CPU_CAPABILITY.OS_AVX512 && CPU_CAPABILITY.HW_AVX512_DQ){
        uint64_t block = iterations / 16 * 16;
        if (block > 0 && seed_scan_maxPID_unroll16_AVX512(rolls, desired_pid, start_seed, block)){
            return true;
        }
        start_seed += block * 0x100000000;
        iterations -= block;
    ...
    if (CPU_CAPABILITY.OS_AVX && CPU_CAPABILITY.HW_AVX2){
        uint64_t block = iterations / 8 * 8;
        if (block > 0 && seed_scan_maxPID_unroll8_AVX2(rolls, desired_pid, start_seed, block)){
            return true;
        }
        start_seed += block * 0x100000000;
        iterations -= block;
    ...
    if (CPU_CAPABILITY.HW_SSE41){
        uint64_t block = iterations / 4 * 4;
        if (block > 0 && seed_scan_maxPID_unroll4_SSE41(rolls, desired_pid, start_seed, block)){
            return true;
        }
        start_seed += block * 0x100000000;
        iterations -= block;
    ...
    if (iterations > 0){
        return seed_scan_maxPID_Default(rolls, desired_pid, start_seed, iterations);
    }
    return false;
}
    ...
    EcPidMatchReporter reporter(stats);
    {
        DynamicParallelizer session(
            [&](uint64_t start, uint64_t length){
                search_candidates_normalPID(reporter, rolls, stats.pid, (uint32_t)(stats.ec - 0x82A2B175229D6A5B) + (start << 32), length);
            },
...
std::vector<Result> run_search_maxPID(const PokemonStats& stats, size_t rolls, size_t threads){
    EcPidMatchReporter reporter(stats);
    {
        DynamicParallelizer session(
            [&](uint64_t start, uint64_t length){
                search_candidates_maxPID(reporter, rolls, stats.pid, (uint32_t)(stats.ec - 0x82A2B175229D6A5B) + (start << 32), length);
            },
            65536, 0x100000000, threads
        );
    }
